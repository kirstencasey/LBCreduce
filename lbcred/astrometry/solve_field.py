...
def solve_field(path_or_pixels, header=None, run_label=None, clean=True,
                save_objs_plot=False, pixscale_limits=[0.2, 0.3], 
    ...
    search_radius: float (optional)
        If target_radec is given, search within this radius in degrees. If 
        None, will use the upper limit of the image scale. 
    ...
    cmd = 'solve-field --cpulimit 30 -u arcsecperpix --fits-image --overwrite'
    cmd = cmd + ' --tweak-order {} --objs {} -L {} -H {}'
    cmd = cmd.format(sip_order, max_num_objects, *pixscale_limits)
    ...
    if target_radec is not None:
        ...
        radius = search_radius if search_radius is not None else image_scale[1]
        ra, dec = target_sc.ra.deg, target_sc.dec.deg
        cmd += f' --ra {ra} --dec {dec} --radius {radius}'
    ...
    if os.path.isfile(prefix + '.solved'):
        ...
        results = ResultStruct(header=_header, fitsio_header=fitsio_header, 
                               pam=pam, success=success)
    else:
        logger.warning('Astrometry failed for ' + base_fn)
        results = ResultStruct(success=success)
