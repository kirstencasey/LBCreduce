...
@utils.func_timer
def register_images(tmp_path, bandpass, index_path=None, ref_cat=None, make_plots=True, config_fn=None):

    if type(config_fn) is not dict:
        with open(config_fn, 'r') as config:
            config = yaml.load(config, Loader=yaml.FullLoader)
    else: config = config_fn
    ...
    for ftype, files, num in zip(['cali', 'sci'], [files_cali, files_sci], [num_cali, num_sci]):
        ...
        options = {'color1' : {'psf' : config['psf_fn_r'], 'zpt' : config['zpt_r'], 'artpop_band' : config['artpop_band_r'], 'artpop_model_fn' : os.path.join(config['out_dir'],ftype,'artpop_model_r.fits'), 'extinction' : config['extinction_r'], 'color_term' : config['colorterm_r']},
        'color2' : {'psf' : config['psf_fn_b'], 'zpt' : config['zpt_b'], 'artpop_band' : config['artpop_band_b'], 'artpop_model_fn' : os.path.join(config['out_dir'],ftype,'artpop_model_b.fits'), 'extinction' : config['extinction_b'], 'color_term' : config['colorterm_b']},
        ...
        for fn in tqdm(files):
            ...
            if config['inject_artpop']:
                ...
                image = misc.inject_model(fn, os.path.join(config['out_dir'],ftype,artpop_fn), config['ypos_inject'], config['xpos_inject'])
                io.write_pixels(fn, image, header=fits.open(fn)[config['ext']].header)

            if config['subtract_background']:
                logger.info('Subtracting background for ' + fn)
                background_subtraction(filename = fn, config = config)
...
def calibrate_images(config, stack):
    ...
    for ftype in ['cali','sci']:
        ...
        if stack:
            ...
            hdu_r.header['SKY_MEAN'] = np.mean(back_vals_r)
            hdu_b.header['SKY_MEAN'] = np.mean(back_vals_b)
            ...
            hdu = fits.PrimaryHDU(mean_stack_r,header=hdu_r.header)
            hdu.writeto(os.path.join(config['image_dir'],ftype,config['final_stack_fn_r']),overwrite=True)
            hdu = fits.PrimaryHDU(mean_stack_b,header=hdu_b.header)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s','--stack-images', default=False)
    ...
    if args.register_images:
        ...
        config = register_images(tmp_path=args.tmp_path, bandpass='R', ref_cat=args.ref_cat, index_path=args.index_path, make_plots=args.make_plots, config_fn=args.config_fn)

        # Register b-band images
        config = register_images(tmp_path=args.tmp_path, bandpass='B', ref_cat=args.ref_cat, index_path=args.index_path, make_plots=args.make_plots, config_fn=args.config_fn)
    ...
    if args.calibrate_images:
        ...
        calibrate_images(config=config, stack=args.stack_images)
