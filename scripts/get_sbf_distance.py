...
def get_sbf_distance(config_fn, model_feh=None) : 
    ...
    mag2_targetgal = np.load(open(os.path.join(config['image_dir'],'background_subtraction_test_results_measured_mags_b'), "rb"))
    mag1_targetgal = np.load(open(os.path.join(config['image_dir'],'background_subtraction_test_results_measured_mags_r'), "rb"))
    sbf_measured_targetgal = np.load(open(os.path.join(config['image_dir'],'background_subtraction_test_results_measured_sbfmags'), "rb"))
    sbf_uncertainty_targetgal = np.load(open(os.path.join(config['image_dir'],'background_subtraction_test_results_sbf_mag_uncertainty'), "rb"))
    ...
    # For each sp in Bessell MIST isos, calculate the g-r color and Mr SBF magnitude using the conversions, Carlsten relation
    gr_calculated = np.asarray(colors_mist_arrs) - BR_gr_conversion
    Mr_gr_calculated = 4.21*gr_calculated - 3.0
    
    # Convert Mr to MR using conversion
